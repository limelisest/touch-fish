...
- 快捷键恢复目标后暂停自动最小化，必须等光标进入目标或衍生窗口并再次离开后才重新计时
- 关闭设置窗口后驻留系统托盘
...
- 每个目标可独立启用 120 × 40 悬浮窗，支持点击或停留 0.3 秒触发、高 DPI 拖动、屏幕边缘吸附，以及多个小悬浮窗之间的吸附对齐，且不会出现在 Alt+Tab 列表
...
- 阅读模块复用 120 × 40 小悬浮窗，支持点击或停留 0.3 秒触发；小窗与阅读窗口位置独立，阅读窗口支持 1 秒进入保护和可配置离开回收时间
