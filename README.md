...
- 修改规则名称和标题关键词
- 删除、刷新和定位目标窗口
- 快捷键按当前整体状态统一全部最小化或统一全部显示；仅控制已配置目标，不包含 TouchFish 阅读窗口
- 只有从小悬浮窗触发的目标直接使用自动最小化逻辑；通过快捷键、Alt+Tab、任务栏或其他系统方式显示/切入时，必须等光标进入目标或衍生窗口并再次离开后才重新计时
- 关闭设置窗口后驻留系统托盘
...
- 每个目标可独立启用 120 × 40 悬浮窗，支持点击或停留 1 秒触发；停留计时期间按下鼠标只进入拖动，不触发目标；并支持高 DPI 拖动、屏幕边缘吸附、多个小悬浮窗之间的吸附对齐，且不会出现在 Alt+Tab 列表
...
- 阅读模块复用 120 × 40 小悬浮窗，支持点击或停留 1 秒触发；小窗与阅读窗口位置独立，阅读窗口支持 1 秒进入保护和可配置离开回收时间
