...
- 每个目标可独立开关光标移开自动最小化，默认 60 秒；光标位于目标或同进程/所属衍生窗口内会重置计时，0 秒会保留 1 秒光标过渡保护后再最小化
- 每个目标可独立启用 120 × 40 悬浮窗，支持点击或停留 0.3 秒触发、高 DPI 拖动和屏幕边缘吸附
...
- 阅读模块复用 120 × 40 小悬浮窗：鼠标移入时显示正文并隐藏小窗，鼠标离开正文后恢复小窗；两者中心位置绑定
